...
#IC mean and covariance matrix of normal distribution
...
#one IC dataset
N = 1000 #data size can be changed to get the results in Figure 5
np.random.seed(0)
Data_raw = multivariate_normal.rvs(mean=Mu, cov=Sigma, size=N)
X_cat = cont_to_cate(Data_raw, Th, P, d)
X_set = cate_to_set(X_cat, N, P)
...
#Mu_oc and Sigma_oc can be set as their IC values to get the IC ARLs in Figure 7
#Mu_oc and Sigma_oc can be set according to OC Cases 1 to 4 to get the OC ARLs in Figure 8
Mu_oc = np.zeros(P)
...
#one example of OC covariance matrix
#for i in np.arange(35,50):
#    for j in np.arange(35,50):
#        Sigma_oc[i,j] = (rho+0.25)**(np.abs(i-j))
        
#Sigma_oc[0:50,0:50] = Sigma_oc[0:50,0:50] * 0.5
        

RL_oc = np.zeros((n_rl, 6))
for method_idx in np.arange(6):
    print(method_idx)
    RL_oc_one = arl_oc(Mu_oc, Sigma_oc, FS, FI_group_idx, FI_num, K, Weight, win_size, 
                       const_a, const_c, Alpha_hat[method_idx], n_rl, method_idx)
    RL_oc[:, method_idx] = RL_oc_one
